...
- 2026-02 — Added **`specialRules: [{name, description}]`** array to all 7 non-Astartes factions (Solar Auxilia: Legate Commanders + Armoured Spearhead; Imperialis Militia: Discipline Masters + Provenance; Knight Households: Ion Gauntlet + Ion Shield; Legio Titanicus: Void Shields; Mechanicum Taghmata: Automaton + Cortex Controller / Cybernetica Cortex; Legio Custodes: The Emperor's Chosen; Daemons of the Ruinstorm: Instability + Chaos Gate + Daemonic Assault). Builder now renders a "// SPECIAL RULES" panel directly under "// LEGION TRAIT" (red border to distinguish from gold), data-testid `special-rules` + `special-rule-${idx}`. Astartes factions correctly omit the box (Legion Trait already covers them). 3/3 new pytest pass + UI smoke screenshot verified.
- 2026-02 — Upgraded **Print View / Field Roster Manifest**. Each formation now prints a full unit stat table (type, speed, armour, CC, FF, weapons range + firepower, notes). Resolves base-option units + extraUnits + addedUnits from flag upgrades + multi-variant upgrades whose variants are real units (e.g. Leman Russ Demolisher from "Infantry Support Tank"). Conversion-flagged Titan/Knight weapon upgrades correctly DO NOT duplicate the parent unit (verified: Warhound Hunting Pack shows 1 stat block, not 4). Print View also renders the Legion Trait box + Special Rules box at the top of the document. data-testid: `print-formation-${i}`, `print-formation-${i}-stats`, `print-unit-<name>`, `print-special-rules`. Smoke-verified (Solar Aux roster: 5 stat blocks including Ogryn addedUnit + Leman Russ multi-variant; Titan roster: 1 stat block).
- 2026-02 — Reworked Print View header into a **two-button export bar**: "Print" (data-testid `print-trigger`) opens the browser's native print dialog; "Save as PDF" (data-testid `save-pdf-trigger`) lazy-loads `html2pdf.js` and downloads a real `.pdf` file (filename = `<sanitised-army-name>-roster.pdf`). Improved print stylesheet: A4 page, black-ink-on-white forced palette, `page-break-before: auto` between formations so unit-stat tables don't split across pages. Verified via Playwright (download event fires; filename = `pdf-export-test-roster.pdf`).
- 2026-02 — Fixed unresponsive Print / Save-as-PDF buttons when the app is loaded inside the **Emergent preview iframe** (cross-origin sandbox blocks `window.print()` and file downloads). `PrintView.jsx` now detects an embedded iframe via `window.top !== window.self`, renders a red "// PREVIEW FRAME DETECTED" banner with an **"Open in New Tab"** button (data-testid `iframe-banner` / `open-fullscreen-btn`), and reroutes Print + Save-as-PDF clicks through `window.open(...)` instead of failing silently. `safeFilename` wrapped in `useMemo` to silence the React-Compiler advisory lint. Verified via Playwright dual-mode test (direct nav → no banner, both buttons present; iframe nav → banner + Open-in-New-Tab button present).
- 2026-02 — Fixed "Army Not Found" on the new tab opened from the preview iframe. Modern browsers (Chrome partitioned storage) isolate `localStorage` between an embedded preview iframe and a top-level tab on the same origin, so the new tab couldn't see armies. Added `/app/frontend/src/lib/share.js` (`encodeArmy` / `decodeArmy` — URL-safe base64 of the army JSON). `openInNewTab` now appends `?fullscreen=1&a=<base64>` to the popup URL; `PrintView` initializes its army state from `URLSearchParams("a")` first and falls back to `getArmy(id)`. After hydration a `useEffect` strips the long `a=` payload via `history.replaceState` so the address bar stays clean. Verified end-to-end via Playwright: iframe → banner click → captured URL contains payload → new-tab navigation with localStorage cleared still renders full Field Roster Manifest (army name, legion trait, points, Print + Save-as-PDF buttons), URL auto-cleans to `/print/<id>?fullscreen=1`.
- 2026-02 — Print / Save-as-PDF in the new tab now finishes in **one click**. Browsers strip transient user activation across the tab boundary, so an automatic `window.print()` in the new tab is silently blocked. Instead, the iframe banner appends `?action=print|pdf` to the new-tab URL, and `PrintView` renders a full-screen prompt overlay (`data-testid='action-prompt-overlay'`) with a giant auto-focused "Start Print" / "Download PDF" button (`action-prompt-confirm`) and a cancel link (`action-prompt-cancel`). That single user click carries the real gesture token, so `window.print()` and the html2pdf download both fire reliably. Verified via Playwright: `?action=print` → overlay → click confirm → `beforeprint`/`afterprint` fire; `?action=pdf` → overlay → click confirm → real `.pdf` download captured; no `?action` → overlay absent (standard roster view).
